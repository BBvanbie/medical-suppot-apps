...
test("ADMIN monitoring drill-down opens filtered case workbench", async ({ page }) => {
  await loginAs(page, testUsers.admin, "/admin/monitoring");

  await expect(page.getByTestId("admin-monitoring-focus-list")).toBeVisible();
  await page.getByRole("link", { name: /選定停滞/ }).click();
  await expect(page).toHaveURL(/\/admin\/cases\?problem=selection_stalled/);
  await expect(page.getByTestId("admin-case-context-note")).toContainText("選定停滞");
});
...
test("HOSPITAL consult view shows judgement checkpoints", async ({ page }) => {
  await loginAs(page, testUsers.hospitalA, "/hospitals/consults");

  const firstConsultButton = page.getByTestId("hospital-consult-open-button").first();
  await expect(firstConsultButton).toBeVisible();
  await firstConsultButton.click();

  await expect(page.getByTestId("hospital-consult-view-modal")).toBeVisible();
  await expect(page.getByText("CONSULT CHECKPOINTS")).toBeVisible();
  await expect(page.getByText("JUDGEMENT CHECK")).toBeVisible();
});
