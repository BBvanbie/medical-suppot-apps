    ...
    <EmsPortalShell operatorName={operator.name} operatorCode={operator.code} currentMode={user.currentMode}>
      <AnalyticsPageLayout
        header={<EmsPageHeader
          ...
          title="救急隊統計"
          description={user.currentMode === "TRAINING" ? "訓練モードでは本番統計を表示しません。training フロー確認は一覧と送信履歴で行ってください。" : "最近の自隊傾向を、時間、種別、年齢帯で比較しながら、搬送決定までの流れを読み取るための統計画面です。"}
          chip={data?.rangeLabel ?? "TRAINING"}
          actions={[
            { label: "ホーム", href: "/paramedics", variant: "secondary" },
            { label: "病院検索", href: "/hospitals/search", variant: "primary" },
          ]}
        />}
        tabs={user.currentMode === "TRAINING" ? null : <AnalyticsRangeTabs basePath="/paramedics/stats" activeRange={range} />}
        ...
        />}
        summary={user.currentMode === "TRAINING" || !data ? null : <EmsMetricStrip title="RANGE SUMMARY" items={data.kpis.map((item) => ({ label: item.label, value: item.value, hint: item.hint }))} />}
      >
