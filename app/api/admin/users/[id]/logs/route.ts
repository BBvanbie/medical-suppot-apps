...
export async function GET(_req: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const logs = await listAdminAuditLogs("user", id);
    return NextResponse.json({ logs });
  } catch (error) {
    console.error("GET /api/admin/users/[id]/logs failed", error);
    return NextResponse.json({ message: "Failed to fetch user logs." }, { status: 500 });
  }
}
