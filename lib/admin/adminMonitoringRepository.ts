...
export async function getAdminMonitoringData(): Promise<AdminMonitoringData> {
  ...
  const [loginAttemptsExists, lockedUntilExists, monitorEventsExists, backupReportsExists] = await Promise.all([
    tableExists("login_attempts"),
    columnExists("users", "locked_until"),
    tableExists("system_monitor_events"),
    tableExists("backup_run_reports"),
  ]);

  const [loginFailures15m, lockedUsers, monitorCounts, recentEvents, topFailureSources, topRateLimitSources, topSecuritySignalSources, backup] =
    await Promise.all([
      loginAttemptsExists
        ? db
            .query<{ count: string }>(
      ...
      monitorEventsExists
        ? db
            ...
            .then((res) => res.rows.map((row) => ({ source: row.source, total: Number(row.total) })))
        : Promise.resolve([]),
      monitorEventsExists
        ? db
            .query<{ source: string; total: string }>(
  ...
  return {
    ...
    loginFailures15m,
    lockedUsers,
    apiFailures24h: Number(monitorCounts?.api_failures_24h ?? "0"),
    notificationFailures24h: Number(monitorCounts?.notification_failures_24h ?? "0"),
    rateLimitHits1h: Number(monitorCounts?.rate_limit_hits_1h ?? "0"),
    securitySignals24h: Number(monitorCounts?.security_signals_24h ?? "0"),
    backup: {
