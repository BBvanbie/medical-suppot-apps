...
| `hospitalCache` | 病院検索キャッシュ | 1日、AES-GCM 暗号化 |
...
- 新規端末登録では端末キー平文を DB に保存せず、`registered_device_key_hash` で照合する。
- UI / API / audit には完全な端末キーを出さず、照合用 fingerprint のみを表示する。

## 暗号化方針

IndexedDB の暗号化は Web Crypto AES-GCM を使う。暗号化対象は患者情報や搬送要請を含む `caseDrafts` と `offlineQueue`、病院検索キャッシュの `hospitalCache` とする。
...
- `hospitalCache` は患者情報を直接含まないが、施設名、電話番号、診療科、位置情報由来の検索キャッシュを端末に残すため、2026-04-12 に暗号化対象へ追加した。
