...
本書は、現在の認証実装を変更・点検するときの実装指示書である。現行方針は `username + password` を基本認証とし、EMS / HOSPITAL ではログイン後に WebAuthn MFA と端末登録を必須にする。
...
|---|---|---|---|
| EMS | 必須 | 必須 | iPad などの現場端末を登録する |
| HOSPITAL | 必須 | 必須 | 病院 PC を登録する |
| ADMIN | 現時点では任意 | 不要 | step-up MFA を後から入れられる構造を維持する |
| DISPATCH | 現時点では任意 | 不要 | 将来 MFA 必須化できるようにする |
...
- EMS / HOSPITAL では `mfaRequired` と `deviceEnforcementRequired` を session に反映する。
...
- ADMIN の通常ログイン MFA は現段階では必須化しない。
...
- EMS / HOSPITAL は `username + password` 後、WebAuthn MFA が完了しない限り保護画面に進めない。
- EMS / HOSPITAL は端末登録が完了しない限り保護画面に進めない。
- ADMIN / DISPATCH は現時点で MFA 未設定でも通常画面へ入れる。
