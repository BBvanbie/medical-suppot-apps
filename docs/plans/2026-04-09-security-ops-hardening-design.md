...
> 2026-04-11 追記: 本書の PIN 再開、3時間無操作、8時間完全再ログインの記述は履歴です。現行仕様では EMS / HOSPITAL は WebAuthn MFA と端末登録を必須とし、PIN 再開導線は使いません。現行方針は `docs/auth_requirements_definition.md` と `docs/auth_implementation_instruction.md` を参照してください。
...
- 無操作 3 時間で「完全ログアウト」ではなく「PIN による再入場」へ移行する案は廃止済み。
- セッション開始から 8 時間経過したら完全再ログインを必須にする案は廃止済み。現行は 5 時間。
- 端末紛失・アカウント停止・パスワード再設定時は、既存セッションを無効化する。
- PIN は 6 桁とする案は廃止済み。
- PIN 入力失敗は 5 回で一時ロックする案は廃止済み。

#### 当時の未決事項（現行では廃止）

- 3 時間無操作時の PIN ロック画面の挙動は対象外。
- 旧 8 時間上限の完全再認証は対象外。現行は 5 時間。
