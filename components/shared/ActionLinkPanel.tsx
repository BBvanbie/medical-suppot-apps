"use client";

import Link from "next/link";
import type { ReactNode } from "react";

type ActionLinkItem = {
  href: string;
  label: string;
  description: string;
  icon: ReactNode;
};

type ActionLinkPanelProps = {
  kicker: string;
  title: string;
  badge?: string;
  items: ActionLinkItem[];
  dataTestId?: string;
  columnsClassName?: string;
  panelClassName?: string;
  itemClassName?: string;
  itemIconClassName?: string;
  kickerClassName?: string;
  titleClassName?: string;
  badgeClassName?: string;
  itemLabelClassName?: string;
  itemDescriptionClassName?: string;
};

export function ActionLinkPanel({
  kicker,
  title,
  badge,
  items,
  dataTestId,
  columnsClassName = "sm:grid-cols-2",
  panelClassName = "ds-radius-hero bg-white px-5 py-5 ds-shadow-panel-warm",
  itemClassName = "group ds-radius-section bg-slate-50/90 px-4 py-3 transition hover:bg-slate-100",
  itemIconClassName = "mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl bg-white text-slate-700",
  kickerClassName = "ds-text-xs-compact font-semibold ds-track-eyebrow-wide text-slate-400",
  titleClassName = "mt-1 text-lg font-bold tracking-tight text-slate-900",
  badgeClassName = "rounded-full bg-slate-100 px-3 py-1 ds-text-xs-compact font-semibold text-slate-600",
  itemLabelClassName = "text-sm font-semibold text-slate-900",
  itemDescriptionClassName = "mt-1 ds-text-xs-compact leading-5 text-slate-500",
}: ActionLinkPanelProps) {
  return (
    <section className={panelClassName} data-testid={dataTestId}>
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <p className={kickerClassName}>{kicker}</p>
          <h2 className={titleClassName}>{title}</h2>
        </div>
        {badge ? <span className={badgeClassName}>{badge}</span> : null}
      </div>
      <div className={`grid gap-2.5 ${columnsClassName}`.trim()}>
        {items.map((item) => (
          <Link key={item.href} href={item.href} className={itemClassName}>
            <div className="flex items-start gap-3">
              <div className={itemIconClassName}>
                {item.icon}
              </div>
              <div className="min-w-0">
                <p className={itemLabelClassName}>{item.label}</p>
                <p className={itemDescriptionClassName}>{item.description}</p>
              </div>
            </div>
          </Link>
        ))}
      </div>
    </section>
  );
}
