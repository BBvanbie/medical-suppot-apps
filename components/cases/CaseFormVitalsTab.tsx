      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <div className="space-y-4">
                ...
                {activeVital.consciousnessType === "jcs" ? (
                  ...
                  <div className={`col-span-1 rounded-lg border p-2 ${consciousnessActive ? "border-blue-300 bg-blue-50/40" : "border-slate-200"}`}>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <select aria-label="GCS E" value={gcsParts.e} onChange={(e) => updateVital("consciousnessValue", composeGcsValue(e.target.value, gcsParts.v, gcsParts.m))} className="rounded-md border border-slate-200 px-2 py-2 text-sm">
                        <option value="">-</option>
                      ...
                      <select aria-label="GCS V" value={gcsParts.v} onChange={(e) => updateVital("consciousnessValue", composeGcsValue(gcsParts.e, e.target.value, gcsParts.m))} className="rounded-md border border-slate-200 px-2 py-2 text-sm">
                        <option value="">-</option>
                      ...
                      <select aria-label="GCS M" value={gcsParts.m} onChange={(e) => updateVital("consciousnessValue", composeGcsValue(gcsParts.e, gcsParts.v, e.target.value))} className="rounded-md border border-slate-200 px-2 py-2 text-sm">
                        <option value="">-</option>
          ...
          <div className="grid grid-cols-12 gap-3">
            ...
            <label className="col-span-3"><span className="mb-1 block text-xs font-semibold text-slate-500">異常呼吸形態</span><select value={activeVital.respiratoryPattern} onChange={(e) => updateVital("respiratoryPattern", e.target.value)} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm">{respiratoryPatterns.map((option) => <option key={option}>{option}</option>)}</select></label>
            <label className="col-span-3"><span className="mb-1 block text-xs font-semibold text-slate-500">呼気臭</span><select value={activeVital.breathOdor} onChange={(e) => updateVital("breathOdor", e.target.value)} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm">{breathOdors.map((option) => <option key={option}>{option}</option>)}</select></label>
          </div>
          ...
          <div className="grid grid-cols-12 gap-3">
            ...
            <label className="col-span-3"><span className="mb-1 block text-xs font-semibold text-slate-500">心電図</span><select value={activeVital.ecg} onChange={(e) => updateVital("ecg", e.target.value)} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm">{ecgOptions.map((option) => <option key={option}>{option}</option>)}</select></label>
            ...
            <div className="col-span-8 rounded-lg border border-slate-200 p-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <p className="mb-1 text-[11px] font-semibold text-slate-500">右</p>
                  <div className="grid grid-cols-3 gap-2">
                    <input type="text" inputMode="decimal" value={activeVital.pupilRight} onChange={(e) => updateVital("pupilRight", formatPupilInput(e.target.value))} className="rounded-lg border border-slate-200 px-2 py-2 text-sm" />
                    <select value={activeVital.lightReflexRight} onChange={(e) => updateVital("lightReflexRight", e.target.value)} className="rounded-lg border border-slate-200 px-2 py-2 text-sm">{lightReflexOptions.map((option) => <option key={option}>{option}</option>)}</select>
                    <select value={activeVital.gazeRight} onChange={(e) => updateVital("gazeRight", e.target.value)} className="rounded-lg border border-slate-200 px-2 py-2 text-sm">{gazeOptions.map((option) => <option key={option}>{option}</option>)}</select>
                  </div>
                </div>
                <div>
                  <p className="mb-1 text-[11px] font-semibold text-slate-500">左</p>
                  <div className="grid grid-cols-3 gap-2">
                    <input type="text" inputMode="decimal" value={activeVital.pupilLeft} onChange={(e) => updateVital("pupilLeft", formatPupilInput(e.target.value))} className="rounded-lg border border-slate-200 px-2 py-2 text-sm" />
                    <select value={activeVital.lightReflexLeft} onChange={(e) => updateVital("lightReflexLeft", e.target.value)} className="rounded-lg border border-slate-200 px-2 py-2 text-sm">{lightReflexOptions.map((option) => <option key={option}>{option}</option>)}</select>
                    <select value={activeVital.gazeLeft} onChange={(e) => updateVital("gazeLeft", e.target.value)} className="rounded-lg border border-slate-200 px-2 py-2 text-sm">{gazeOptions.map((option) => <option key={option}>{option}</option>)}</select>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-12 gap-3">
            <div className="col-span-3">
              ...
              <input type="text" inputMode="decimal" value={activeVital.temperature} disabled={activeVital.temperatureUnavailable} onChange={(e) => updateVital("temperature", formatTemperatureInput(e.target.value))} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm disabled:bg-slate-100" />
              <button
                type="button"
                onClick={() => {
            ...
            return (
              <div key={major.id} className="rounded-xl border border-slate-200">
                <button
                  ...
                  className={`flex w-full items-center justify-between px-4 py-3 text-left ${majorChanged ? "bg-amber-50/80" : ""}`}
                >
                          ...
                          <div key={middleId} className="rounded-lg border border-slate-200 bg-slate-50/50">
                            ...
                            {middleOpen ? (
                              <div className="border-t border-slate-200 px-3 py-2">
                                {renderFindingBody(major.id, middle.id, middle.children)}
                              </div>
                            ) : null}
                          </div>
                        );
