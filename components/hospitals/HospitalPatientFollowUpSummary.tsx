...
export function HospitalPatientFollowUpSummary({
  ...
  return (
    <section className="ds-panel-surface rounded-2xl p-6">
      <p className="text-xs font-semibold uppercase ds-track-eyebrow-wide text-emerald-600">FOLLOW-UP CHECK</p>
      <div className="mt-4 grid gap-3 lg:grid-cols-3">
        <div className="rounded-2xl bg-emerald-50/70 px-4 py-4">
          <p className="ds-text-2xs font-semibold ds-track-eyebrow text-emerald-700">CURRENT STATUS</p>
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <RequestStatusBadge status={status} />
            {prioritySummary ? (
              <span className="rounded-full bg-white px-2.5 py-1 ds-text-2xs font-semibold text-emerald-700">
                {prioritySummary}
              </span>
            ) : null}
          </div>
        ...
        <div className="rounded-2xl bg-slate-50/90 px-4 py-4">
          <p className="ds-text-2xs font-semibold ds-track-eyebrow text-slate-500">TRACKING CONTEXT</p>
